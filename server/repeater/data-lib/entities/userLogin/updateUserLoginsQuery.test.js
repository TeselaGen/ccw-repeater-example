...
describe("updateUserLoginsQuery", () => {
    it("updateUserLoginsQuery updates records via a query", async () => {
        jest.clearAllMocks();
        ...
        let result = await updateUserLoginsQuery.call(ctx, statements);
    ...
    it("updateUserLoginsQuery returns empty array if disallowed", async () => {
        let ctx = {
            db: () => ({ count: () => "qry" }),
            entities: {
                userLogin: {
        ...
        let result = await updateUserLoginsQuery.call(ctx);
