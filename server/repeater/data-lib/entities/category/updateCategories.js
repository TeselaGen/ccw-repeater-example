...
async function updateCategories(updateRecords, fkFilter, trx, opts){

    let allowQuery = await Promise.resolve(this.entities.category.extensions.onUpdate.allow.call(this, opts));

    if(!allowQuery) return Promise.resolve([]);

    let records = await Promise.resolve(this.entities.category.extensions.onUpdate.records.call(this, updateRecords, opts));
    ...
    return Promise.each(records, async (updateRecord) =>
    {
        ...
        if(startsWith(recordId, "&")){
            let rec = await qry("category").first("id").where({ cid: recordId.substr(1) });
            recordId = rec.id;
        ...
        updateFilter = await Promise.resolve(dataLib.entities.category.extensions.onUpdate.filter.call(dataLib, updateFilter, opts));

        let additionalFilter = await Promise.resolve(this.entities.category.extensions.onUpdate.appendFilter.call(this, opts));
        ...
        return applyFilter(qry("category"), "category", updateFilter)
                .update(values)
                .then(() => {
                    return applyFilter(qry("category").select(select), "category", updateFilter)
                           .first();
                })
                .then((result) => {
                    if(result) ids.push(result.id);
    ...
    .then(() => {
        return this.entities.category.updateNestedRecords(qry, records);
    })
    .then(() => {
